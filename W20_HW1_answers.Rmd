...
# What is the current value of the object height_cm? 182.88
# What is the current value of the object height_in? 72
```
...
# What is the current value of the object height_cm? 167.64
# What is the current value of the object height_in? 66
```

R can also evalute objects, it can determine if their values are equal.
```{r}
# Are these values equal? No
height_in == height_cm
# Are these values different? Yes
height_in != height_cm
```
...
### Challenge
What are the values after running statement in the following code block? Enter the answers as comments. 
```{r}
mass <- 81.6           # mass? 81.6
age  <- 21             # age? 21
...
Here, we’ve called mean() with just two arguments; a vector containing the numbers, and what the function should do if it encounters and NA value (more on NAs later). 
...
# There is one number in quoatation marks as a character, so it converts all of them to characters
```
...
# One TRUE value, because the TRUE value in num_logical is converted into a 1 for the numeric vector
```

Q1 = 1 point for upper panel answers. 1 point for lower panel answers. 2 points total.
...
Q2 = 1 point for correct answer. 

# Question 3
```{r}
# Build a vector of the study semesters (Winter 2015 through Winter 2020): 

study_semester <- c("Winter 2015","Fall 2015",
...
Q3 = 1.5 points for both vectors (0.75 points each), 0.5 points for type answer. 

# Question 4
```{r}
# Build a vector of the typical age of college students:

college_age <- c(17,18,19,20,21,22,23)
...
college_age <- as.character(college_age)
class(college_age)

```

Q4 = 1 point for creating numeric vector, 1 point for conversion. 
...
# Conversion and Confirmation of pH_1
pH_1 <- c("7.75", "6", "7.5", "8", "7.75")
numeric_pH_1 <- (as.numeric(pH_1))
class(numeric_pH_1)


# Conversion and Confirmation of pH_2
pH_2 <- c("8", "7.25", "7.25", "7.75", "7.5", "7.25")
numeric_pH_2 <- (as.numeric(pH_2))
class(numeric_pH_2)

# Mean Calculation
mean_pH_1 <- round((mean(numeric_pH_1)), digits = 2)
mean_pH_1
mean_pH_2 <- (mean(numeric_pH_2))
mean_pH_2

# Equality
mean_pH_1 == mean_pH_2 #FALSE
```
 Q5 = 2 for conversion and confirmation of each vector. 0.5 for mean calculation. 0.5 for equality
